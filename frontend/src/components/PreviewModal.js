  ...
  const renderPreviewBlock = (block) => {
    switch (block.type) {
      case 'menu':
        ...
        const menuStyle = block.content?.style || 'light';
        
        return (
          <nav className={`${
            menuStyle === 'dark' ? 'bg-gray-900 text-white' : 
            menuStyle === 'gradient' ? 'bg-gradient-to-r from-indigo-600 to-purple-600 text-white' :
            'bg-white text-gray-900 border-b border-gray-200'
          } py-4 px-6 sticky top-0 z-50 shadow-md`}>
                ...
                {links.map((link, idx) => (
                  <a 
                    ...
                    className={`hover:opacity-75 transition-opacity ${
                      menuStyle === 'light' ? 'text-gray-700 hover:text-gray-900' : ''
                    }`}
                  >
                    {link.text}
                  </a>
